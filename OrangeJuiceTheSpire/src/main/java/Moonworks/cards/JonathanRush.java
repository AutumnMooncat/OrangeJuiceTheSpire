...
import static Moonworks.OrangeJuiceMod.enableSelfDamage;
...
public class JonathanRush extends AbstractDynamicCard {
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        AbstractDungeon.actionManager.addToBottom(new AbstractGameAction() {
            public void update() {
                for (int gulls = 0 ; gulls < Math.max(1, gullThrows / 5f) ; gulls++) {
                    if (hasThorns) {
                        CardCrawlGame.sound.play("ATTACK_FAST");
                    } else {
                        CardCrawlGame.sound.play(damage > 15 ? "BLUNT_HEAVY" : "BLUNT_FAST");
                    }
                }
                //AbstractDungeon.effectList.add(hurtGull);
                this.isDone = true;
